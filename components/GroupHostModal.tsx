...
export function GroupHostModal({ open, settings, onCloseToPersonal }: Props) {
  ...
  async function end() {
    if (!roomId || !hostToken) return;
    try {
      const resEnd = await fetch(`/api/rooms/${roomId}/end`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ hostToken }),
      });
      const endJson = await resEnd.json().catch(() => null);
      if (!resEnd.ok || !endJson?.ok) {
        const msg = String(endJson?.error || "end_failed");
        alert(settings.lang === "ko" ? `종료 실패: ${msg}` : `End failed: ${msg}`);
        return;
      }
      await fetchState();
    } catch {}
