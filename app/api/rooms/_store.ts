...
export async function saveRoom(room: Room): Promise<void> {
  // ✅ update는 row가 없으면 "0행 업데이트"로 조용히 끝날 수 있어요.
  // 서버리스 환경에서 간헐적 not_found/종료 불가/제출 실패처럼 보이는 원인이 됩니다.
  // 그래서 항상 upsert로 저장해 row가 반드시 존재하도록 만듭니다.
  const { error } = await supabaseServer
    .from(TABLE)
    .upsert({ id: room.id, state: room, updated_at: new Date().toISOString() }, { onConflict: "id" });
  if (error) throw new Error(error.message);
}
