...
import { endRoom, getPublicState } from "../../_store";
...
export async function POST(req: Request, ctx: { params: Promise<{ roomId: string }> }) {
  ...
  try {
    const room = await endRoom(roomId, hostToken);
    ...
    return NextResponse.json({ ok: false, error: e?.message ?? "end_failed" }, { status: 400 });
  }
}
